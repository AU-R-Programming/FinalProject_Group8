...
```{r}
ci <- bank_model$bootstrap_intervals
print(ci)
```

Finally, We can evaluate the performance of the logistic regression model using the Confusion Matrix.

```{r}
metrices <- data.frame(
  Variable = names(bank_model$confusion_metrics),  
  Value = as.vector(t(bank_model$confusion_metrics))
)
print(metrices)
