...
Now, the package is available for use. Notice above that the name of the GitHub repository is FinalProject_Group8, and the name of the package is FinalGroup8.
...
Then, we can extract the 95% bootstrap confidence intervals for the coefficients.

```{r}
bank_model$bootstrap_intervals
```

For our bootstrap intervals, the default parameter values were used (`B = 20` and `conf_level = 0.95`), which uses 20 bootstrap samples to create 95% bootstrap confidence intervals for each coefficient. 

For each of the 20 bootstrap samples, a random sample of the original data rows are taken with replacement. Then, using this random sample of data rows, coefficients for each variable are estimated. This is done 20 times to get 20 different estimates for each coefficient. The intervals are then constructed by taking the 2.5th percentile and 97.5th percentile of the 20 values for each coefficient.
...
This confusion matrix gives an estimate on how accurate the logistic regression function is at predicting the true class. It uses the estimated coefficients to predict the class for each data observation. Then the predicted class is compared to the actual class for each observation.
...
Something important to note. When I refer to the "positive" class, I am referring to the class that is in the second column/second row of the confusion matrix (in this case, that is the "yes" class), and when I refer to the "negative" class, I am referring to the class that is in the first column/first row of the confusion matrix (in this case, that is the "no" class).
